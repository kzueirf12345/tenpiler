...
std::string GetDot(const Conv& n){
	return std::to_string(Conv::MIN_INPUTS_SIZE) + "-" + std::to_string(Conv::MAX_INPUTS_SIZE) + " | " + n.meta().op_type + " | " + std::to_string(Conv::MIN_OUTPUTS_SIZE) + "-" + std::to_string(Conv::MAX_OUTPUTS_SIZE);
}

std::string GetDot(const MaxPool& n){
	return std::to_string(MaxPool::MIN_INPUTS_SIZE) + "-" + std::to_string(MaxPool::MAX_INPUTS_SIZE) + " | " + n.meta().op_type + " | " + std::to_string(MaxPool::MIN_OUTPUTS_SIZE) + "-" + std::to_string(MaxPool::MAX_OUTPUTS_SIZE);
}

std::string GetDot(const Gemm& n){
	return std::to_string(Gemm::MIN_INPUTS_SIZE) + "-" + std::to_string(Gemm::MAX_INPUTS_SIZE) + " | " + n.meta().op_type + " | " + std::to_string(Gemm::MIN_OUTPUTS_SIZE) + "-" + std::to_string(Gemm::MAX_OUTPUTS_SIZE);
}
